  ...
  for (let i = 0; i <= 23; i++) {
    for (const [key, value] of Object.entries(weather_code)) {
      if (key == weatherCode) {
        ...
        if (key == 0) {
          $(`#wIconH${index + 1}`).html(
            `<img class="weatherICON" id="hourlyIcon${
              i + 1
            }" src="assets/sun.svg" alt="clear-sky">`
          );
        } else if (key == 1 || key == 2 || key == 3) {
          $(`#wIconH${index + 1}`).html(
            `<img class="weatherICON" id="hourlyIcon${
              i + 1
            }"  src="assets/cloudy.svg" alt="cloudy">`
          );
        } else if (key == 45 || key == 48) {
          $(`#wIconH${index + 1}`).html(
            `<img class="weatherICON" id="hourlyIcon${
              i + 1
            }"  src="assets/fog.svg" alt="Fog">`
          );
        } else if (
          key == 51 ||
          key == 53 ||
          ...
          $(`#wIconH${index + 1}`).html(
            `<img class="weatherICON" id="hourlyIcon${
              i + 1
            }"  src="assets/drizzle.svg" alt="drizzle">`
          );
        } else if (
          key == 61 ||
          key == 63 ||
          ...
          $(`#wIconH${index + 1}`).html(
            `<img class="weatherICON" id="hourlyIcon${
              i + 1
            }"  src="assets/rain.svg" alt="rain">`
          );
        } else if (key == 71 || key == 73 || key == 75 || key == 77) {
          $(`#wIconH${index + 1}`).html(
            `<img class="weatherICON" id="hourlyIcon${
              i + 1
            }"  src="assets/snowy.svg" alt="snowy">`
          );
        } else if (
          key == 80 ||
          key == 81 ||
          ...
          $(`#wIconH${index + 1}`).html(
            `<img class="weatherICON" id="hourlyIcon${
              i + 1
            }"  src="assets/rainshowers.svg" alt="rain showers">`
          );
        } else if (key == 95 || key == 96 || key == 99) {
          $(`#wIconH${index + 1}`).html(
            `<img class="weatherICON" id="hourlyIcon${
              i + 1
            }"  src="assets/thunderstorm.svg" alt="thunderstorm">`
          );
        }
      }
    }
...
function currentIMG(weatherCode) {
  ...
  for (const [key, value] of Object.entries(weather_code)) {
    if (key == weatherCode) {
      ...
      if (key == 0) {
        $("#wIcon").html(
          `<img class="weatherICON" src="assets/sun.svg" alt="clear-sky">`
        );
      } else if (key == 1 || key == 2 || key == 3) {
        $("#wIcon").html(
          `<img class="weatherICON" src="assets/cloudy.svg" alt="cloudy">`
        );
      } else if (key == 45 || key == 48) {
        $("#wIcon").html(
          `<img class="weatherICON" src="assets/fog.svg" alt="Fog">`
        );
      } else if (
        key == 51 ||
        key == 53 ||
        ...
        $("#wIcon").html(
          `<img class="weatherICON" src="assets/drizzle.svg" alt="drizzle">`
        );
      } else if (
        key == 61 ||
        key == 63 ||
        ...
        $("#wIcon").html(
          `<img class="weatherICON" src="assets/rain.svg" alt="rain">`
        );
      } else if (key == 71 || key == 73 || key == 75 || key == 77) {
        $("#wIcon").html(
          `<img class="weatherICON" src="assets/snowy.svg" alt="snowy">`
        );
      } else if (
        key == 80 ||
        key == 81 ||
        ...
        $("#wIcon").html(
          `<img class="weatherICON" src="assets/rainshowers.svg" alt="rain showers">`
        );
      } else if (key == 95 || key == 96 || key == 99) {
        $("#wIcon").html(
          `<img  class="weatherICON" src="assets/thunderstorm.svg" alt="thunderstorm">`
        );
      }
    }
  }
...
function convertToFahrenheitBTN() {
  $("#fahrenheit").addClass("activeTemp");
  $("#celsius").removeClass("activeTemp");
  $(".minTemp, .maxTemp, .currntTemp, .aprtTemp").each(function () {
    ...
    if (!isNaN(tempValue)) {
      ...
      $(this)
        ...
        .text(fahrenheitValue.toFixed(1));
    }
  });
  ...
  for (let i = 0; i < 7; i++) {
    $(`#dailyMax${i + 1}`).text(
      convertToFahrenheit($(`#dailyMax${i + 1}`).text())
    );
    $(`#dailyMin${i + 1}`).text(
      convertToFahrenheit($(`#dailyMin${i + 1}`).text())
    );
  }
  ...
  for (let i = 0; i <= 23; i++) {
    $(`#tempHourlyValue${i + 1}`).html(
      convertToFahrenheit($(`#tempHourlyValue${i + 1}`).text())
    );
    $(`#windHorulyValue${i + 1}`).html(
      kmToMiles($(`#windHorulyValue${i + 1}`).text())
    );
...
function convertToCelsiusBTN() {
  $("#celsius").addClass("activeTemp");
  $("#fahrenheit").removeClass("activeTemp");
  ...
  for (let i = 0; i < 7; i++) {
    $(`#dailyMax${i + 1}`).text(
      convertToCelsius($(`#dailyMax${i + 1}`).text())
    );
    $(`#dailyMin${i + 1}`).text(
      convertToCelsius($(`#dailyMin${i + 1}`).text())
    );
  }
  ...
  for (let i = 0; i <= 23; i++) {
    $(`#tempHourlyValue${i + 1}`).html(
      convertToCelsius($(`#tempHourlyValue${i + 1}`).text())
    );
    $(`#windHorulyValue${i + 1}`).html(
      mphToKmh($(`#windHorulyValue${i + 1}`).text())
    );
...
function processHourlyData(
  ...
  for (let i = 0; i < 7; i++) {
    ...
    let dailyTemp = hourlyTemperature.slice(
      i * HOURS_IN_DAY,
      (i + 1) * HOURS_IN_DAY
    );
    let dailyPrecip = hourlyPrecipitation.slice(
      i * HOURS_IN_DAY,
      (i + 1) * HOURS_IN_DAY
    );
    let dailyWeatherCode = hourlyWeatherCode.slice(
      i * HOURS_IN_DAY,
      (i + 1) * HOURS_IN_DAY
    );
    let dailyWindSpeed = hourlyWindSpeed.slice(
      i * HOURS_IN_DAY,
      (i + 1) * HOURS_IN_DAY
    );
    let dailyHumidity = hourlyHumidity.slice(
      i * HOURS_IN_DAY,
      (i + 1) * HOURS_IN_DAY
    );
...
function getTimezoneForCoordinates(latitude, longitude) {
  ...
  $.getJSON(url, function (whereisthisat) {
    ...
    $(".today").html(
      `${sevenDayForecast.daysOfWeekFULL[0]} ${sevenDayForecast.dates[0]}`
    );

    for (let i = 0; i < 7; i++) {
      $(`#day${i + 1}`).html(
        `<span id="theDay${i + 1}">${
          sevenDayForecast.daysOfWeek[i]
        }</span> <span id="theDate${i + 1}">${
          sevenDayForecast.dates[i]
        }</span>`
      );
    }
  });
...
function updateHourlyData(hourlyTM) {
  ...
  var tempSum = 0;
  var humidSum = 0;
  var selectedHigh = $(".sevenDayContent")
    .children()
    .eq(hourlyTM)
    .find(`#dailyMax${hourlyTM + 1}`)
    .text();
  var selectedLow = $(".sevenDayContent")
    .children()
    .eq(hourlyTM)
    .find(`#dailyMin${hourlyTM + 1}`)
    .text();
  var mostFrequentWeatherCode = findMulti(
    dailyWeatherCodeData[hourlyTM]
  );
  ...
  if ($("span#convrt").hasClass("fahrenheit") == true) {
    for (let i = 0; i <= 23; i++) {
      $(`#tempHourlyValue${i + 1}`).html(
        convertToFahrenheit(dailyTemperatureData[hourlyTM][i])
      );
      //$(`#hour${i+1}code`).html(dailyWeatherCodeData[hourlyTM][i]);
      $(`#percipHorulyValue${i + 1}`).html(
        dailyPrecipitationData[hourlyTM][i]
      );
      $(`#windHorulyValue${i + 1}`).html(
        kmToMiles(dailyWindSpeedData[hourlyTM][i])
      );
    ...
    for (let i = 0; i <= 23; i++) {
      $(`#tempHourlyValue${i + 1}`).html(
        dailyTemperatureData[hourlyTM][i]
      );
      //$(`#hour${i+1}code`).html(dailyWeatherCodeData[hourlyTM][i]);
      $(`#percipHorulyValue${i + 1}`).html(
        dailyPrecipitationData[hourlyTM][i]
    ...
    function (openMeteo) {
      ...
      for (let i = 0; i < openMeteo.hourly.time.length; i++) {
        if (openMeteo.hourly.time[i] == newTime) {
          ...
          var precipitation_probab =
            openMeteo.hourly.precipitation_probability[i];
          ...
          for (let i = 0; i < 7; i++) {
            ...
            for (const [key, value] of Object.entries(weather_code)) {
              if (key == weatherCode) {
                if (key == 0) {
                  $(`#wIcon${i + 1}`).html(
                    `<img class="weatherICON" id="dailyIcon${
                      i + 1
                    }" src="assets/sun.svg" alt="clear-sky">`
                  );
                } else if (key == 1 || key == 2 || key == 3) {
      ...
      if ($("#convrt").hasClass("fahrenheit")) {
        ...
        for (let i = 0; i < 7; i++) {
          $(`#day${i + 1}Temp`).html(
            `<span id="dailyMax${i + 1}">${convertToFahrenheit(
              openMeteo.daily.temperature_2m_max[i]
            )}</span>&deg; <span id="dailyMin${
              i + 1
            }">${convertToFahrenheit(
              openMeteo.daily.temperature_2m_min[i]
            )}</span>&deg;`
          );
        }
      } else {
        ...
        for (let i = 0; i < 7; i++) {
          $(`#day${i + 1}Temp`).html(
            `<span id="dailyMax${i + 1}">${
              openMeteo.daily.temperature_2m_max[i]
            }</span>&deg; <span id="dailyMin${i + 1}">${
              openMeteo.daily.temperature_2m_min[i]
            }</span>&deg;`
          );
        }
...
$(document).ready(() => {
  $(".sevenDayContent")
    ...
    .click(function () {
      ...
      $(".today").html(
        `${whatisToday(clickedDay.toUpperCase())} ${theDATE}`
      );
    });
  ...
  for (let i = 0; i <= 23; i++) {
    if (i <= 11) {
      ...
      <p id="hour${i + 1}Temp"><span id="tempHourlyValue${
        i + 1
      }"></span><span class="tempEmblm"></span></p>
      <div id="wIconH${i + 1}"></div>
      <p id="hour${i + 1}code"></p>
      <p id="hour${
        i + 1
      }precip"><img id="dropImg" src="assets/drop.svg" alt="raindrop"/><span id="percipHorulyValue${
        i + 1
      }"></span>%</p>
      <p id="hour${i + 1}wind"><span id="windHorulyValue${
        i + 1
      }"></span> <span id="hourlySpeedEmblm${i + 1}"></span></p>
      </div>`);
    } else {
  ...
  $.getJSON("https://api.ipify.org?format=json", function (ipIFY) {
    ...
    $.getJSON("http://ip-api.com/json/" + ipIFY.ip, function (ipAPI) {
      var latitude = ipAPI.lat;
      var longitude = ipAPI.lon;
      $(".location").html(
        ipAPI.city + ", " + ipAPI.regionName + ", " + ipAPI.country
      );
...
$(document).ready(() => {
  ...
  $("#globe").click(() => {
    console.log(
      "Please enable location services to get the weather of your location"
    );
      ...
      navigator.geolocation.getCurrentPosition(function (position) {
        ...
        $.getJSON(
          ...
          function (locationIQ) {
            ...
            if (
              locationIQ.address.city == undefined ||
              locationIQ.address.state == undefined
            ) {
              $("#locatCITY").hide();
              ...
              $(".location").html(
                locationIQ.address.city +
                  ", " +
                  locationIQ.address.state +
                  ", " +
...
$(document).ready(() => {
  $("#search-box-input").autocomplete({
    ...
    onSelect: function (suggestion) {
      ...
      if (
        suggestion.data.address.city == undefined ||
        suggestion.data.address.state == undefined
      ) {
        $(".location").html(suggestion.data.display_place);
      } else {
        $(".location").html(
          suggestion.data.address.city +
            ", " +
            suggestion.data.address.state +
            ", " +
            suggestion.data.address.country
        );
      }
      getWeatherData(latitude, longitude, function () {
        updateHourlyData(0);
